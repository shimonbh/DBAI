...
/** Expandable tree node for a database table. Shows columns when expanded. */
export function TableNode({ table, nodeId, expanded, onToggle }: Props) {
  ...
  const insertTable = (e: React.MouseEvent) => {
    e.stopPropagation()
    const quoted = quoteIdentifier(table.name, dbType)
    openTab(`SELECT * FROM ${quoted} LIMIT 100`, table.name)
  }
...
const styles = {
  row:       { display: 'flex', alignItems: 'center', gap: 4, padding: '4px 12px 4px 24px', cursor: 'pointer', userSelect: 'none' as const, '&:hover': { background: theme.bgPanel } },
  indent:    { width: 8 },
  ...
  notNull:   { fontSize: 9, color: '#f38ba8', background: '#2b1010', borderRadius: 3, padding: '0 3px' },
}
